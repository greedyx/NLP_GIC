...
if __name__ == '__main__':
    model1 = MLE(4)
    processed_news_dataframe = pd.read_csv('test.csv')
    # Train as all the bodies chained
    start = time.time()
    print("Fitting model1 for date: 2018-06-01 \n")
    text = processed_news_dataframe.loc[0,'0']
    '''
    train_fourgram_model(gen_daily_data_corpus(processed_news_dataframe, np.datetime64('2020-11-25')), model1)
    '''
    train_fourgram_model(text,model1)
    end = time.time()
    ...
    '''
    model3 = MLE(4)
    weekly_training_corpus = gen_7day_corpus(processed_news_dataframe, np.datetime64('2018-06-08'))
    start = time.time()
